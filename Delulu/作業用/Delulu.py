...
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/アイテム画像/Delulu"
...
def main():
    ...
    try:
        ...
        # 0) Hotfix（必要時のみ実行）
        if DO_HOTFIX:
            subprocess.run([sys.executable, r"Delulu/作業用/LootPackage変更.py"], check=True)
            subprocess.run([sys.executable, r"Delulu/作業用/LootTier変更.py"], check=True)
            print("✓ Hotfix 適用完了")
        ...
        br_lootdata_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/Delulu/LootPercent")
        ...
        try:
            ...
            msg = f"Delulu update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
