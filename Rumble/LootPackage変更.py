...
# ==== 入出力（必要なら名前だけ変えてOK）====
BASE_PATH   = Path("e:/Fmodel/Exports/FortniteGame/Content/Items/DataTables/AthenaLootPackages_Client.json")
SEASON_PATH = Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/LootCurrentSeasonLootPackages_Client.json")
OVERRIDE_PATH = Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/Rumble/OverrideLootCurrentSeasonLootPackagesData_Rumble.json")
OVERRIDE_BACKUP_PATH = Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/Rumble/OverrideLootCurrentSeasonLootPackagesData_Rumble_backup.json")
HOTFIX_PATH = Path("e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")

OUT_FINAL   = Path("E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Rumble/AthenaLootPackages_Client__final.json")         # 最終だけ保存（合成+Hotfix or 合成のみ）
...
def main():
    ...
    print(f"[MERGE SEASON] replaced={rep}, added={add}")
    ...
    rep, add = merge_rows(base_rows, override_meta["Rows"])
    print(f"[MERGE OVERRIDE] replaced={rep}, added={add}")

    # 4) Override Rumble Backup
    override_backup_meta = read_datatable_json(OVERRIDE_BACKUP_PATH)
    rep, add = merge_rows(base_rows, override_backup_meta["Rows"])
    print(f"[MERGE OVERRIDE BACKUP] replaced={rep}, added={add}")
