...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/ORIGIN_LootPool.json"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_figment")
    if logger.handlers:
        return logger
    ...
    logger.info("log file (Figment): %s", log_path)
    return logger
...
def main():
    ...
    try:
        logger.info("===== Figment: pipeline start =====")
        ...
        logger.info("build Figment_LootData start")
        t0 = time.time()
        ...
        logger.info("===== Figment: pipeline end =====")
