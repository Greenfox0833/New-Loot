...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/loot/data/HazelnutSpread_LootPool.json"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_HazelnutSpread")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== HazelnutSpread: pipeline start =====")
        ...
        try:
            ...
            msg = f"HazelnutSpread update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== HazelnutSpread: pipeline end =====")
