...
INPUT_LT_JSON = r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/Updated_LootTier.json"
INPUT_LP_JSON = r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/Updated_LootPackages.json"

# 画像の保存先（親）:  <OUTPUT_BASE_DIR>/<TierGroup>/<WorldListKey>/ に振り分け保存
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/v37.00"
...
import subprocess
if DO_HOTFIX:
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/Package合成.py"], check=True)
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/Tier合成.py"], check=True)
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/LootPackage更新.py"], check=True)
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/LootTier更新.py"], check=True)
# --- Hotfix適用ここまで ---
...
def build_summary(rows_lt: dict, rows_lp: dict):
    ...
    # (A) (LootPackageID, LootPackageCategory) → Package行リスト（.01/.02/.03 すべて保持）
    from collections import defaultdict
    ...
    for row_key, row in rows_lp.items():
        ...
        cand = {
            "Key": row_key,
            "Call": row.get("LootPackageCall", "") or row.get("LootPackageID", "")
        }
        lp_by_idcat[(lp_id, lp_cat)].append(cand)
    ...
    for row_key, row in rows_lp.items():
        wl_key = row.get("LootPackageCall", "") or row.get("LootPackageID", "")
        worldlist_map[wl_key].append({
            "Key": row_key,
            "Weight": row.get("Weight", 0.0),
            "AssetPathName": row.get("ItemDefinition", {}).get("AssetPathName", ""),
            "CountItem": (row.get("CountRange") or {}).get("X")
        })
    for wl_key in worldlist_map:
        worldlist_map[wl_key].sort(key=lambda x: key_suffix_num(x["Key"]))

    # --- ここから追加: WorldList 再帰展開リゾルバ ---
    WL_INDEX = defaultdict(list)
    for row_key, row in rows_lp.items():
        WL_INDEX[row.get("LootPackageID", "")].append({
            "Key": row_key,
            "Weight": row.get("Weight", 0.0),
            "AssetPathName": (row.get("ItemDefinition") or {}).get("AssetPathName", ""),
            "Call": row.get("LootPackageCall", "") or row.get("LootPackageID", ""),
            "CountItem": (row.get("CountRange") or {}).get("X"),
        })

    def resolve_worldlist_items(start_key: str, max_hops: int = 2000):
        """WorldListを再帰展開して最終アイテムまで取得"""
        if not start_key:
            return []
        out = []
        seen = set()
        stack = {start_key, start_key.replace(".", "_"), start_key.replace("_", ".")}
        hops = 0
            ...
            for r in WL_INDEX.get(k, []):
                ...
                if w and ap:
                    out.append({
                        "Weight": w,
                        "AssetPathName": ap,
                        "CountItem": r.get("CountItem"),
                        "Key": r["Key"],
                    })
                else:
                    nxt = r["Call"]
                    if nxt and nxt not in seen and nxt != k:
                        stack.add(nxt)
        out.sort(key=lambda x: key_suffix_num(x.get("Key", "")))
        for i in out:
            i.pop("Key", None)
        return out
    # --- 追加ここまで ---
    ...
    for row_name, row in rows_lt.items():
        ...
        valid_list = []
        seen_calls = set()  # 同じ Call の重複を防ぐ保険（任意）

        for cat_idx, bit in enumerate(weight_array):  # 0,1,2,...
            if bit < 1:
                continue

            matches = lp_by_idcat.get((loot_pkg, cat_idx), [])
            if not matches:
                continue

            for m in matches:  # 例: .01/.02/.03 すべて
                pkg_id = m["Key"]
                call   = m["Call"]

                # dedupe は Call が空なら pkg_id をキーにする
                dedupe_key = call or pkg_id
                ...
                # 修正後（差し替え）
                list_items = resolve_worldlist_items(call)

                # ※もし Category==0 でも中身を見たいなら、次の2行をコメントアウト
                if cat_idx == 0:
                    list_items = []

                # ListItems が空でも捨てない。TotalListWeight は 0 にする
