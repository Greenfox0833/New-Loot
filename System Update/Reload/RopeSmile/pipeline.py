...
WEB_LOOTPOOL_NAME = "RopeSmile_LootPool.json"

# Ensure child scripts resolve profile config at System Update/Reload/RopeSmile
os.environ["SYSTEM_PROFILE"] = "Reload/RopeSmile"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_RopeSmile")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== RopeSmile: pipeline start =====")
        ...
        try:
            ...
            msg = f"RopeSmile update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== RopeSmile: pipeline end =====")
