...
PATH_BB_LT     = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/BlastBerryTierData.json")
PATH_OVR_LT    = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/AthenaLootTierData_Client_BlastBerryOverride.json")

HOTFIX_PATH    = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")

OUT_FINAL      = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/作業用/AthenaLootTierData_Client__final_LT.json")
...
def parse_unreal_tuple_to_dict(s: str) -> Dict[str, Any]:
    ...
    for seg in inner.split(","):
        if "=" not in seg:
            continue
        k, v = seg.split(",", 1) if False else seg.split("=", 1)
...
def main():
    ...
    meta_ovr    = read_datatable_json(PATH_OVR_LT)
    ...
    if hotfix_text:
        apply_hotfix_for_table(rows_athena, hotfix_text, {"AthenaLootTierData_Client", "/Game/Items/Datatables/AthenaLootTierData_Client"})
    else:
        print("[HOTFIX] skipped (file not found)")

    # ② Athena に BlastBerryTierData を上書き
    rep, add = merge_rows(rows_athena, rows_bb)
    print(f"[STEP2] AthenaLT <- BlastBerryLT : replaced={rep}, added={add}")

    # ③ BlastBerryTierData に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(rows_bb, hotfix_text, {"/BlastBerryLoot/DataTables/BlastBerryTierData", "BlastBerryTierData"})

    # ④ BlastBerryTierData に AthenaLootTierData_Client_BlastBerryOverride を上書き
    rep, add = merge_rows(rows_bb, rows_ovr)
    print(f"[STEP4] BlastBerryLT <- LT_Override : replaced={rep}, added={add}")

    # ⑤ AthenaLootTierData_Client_BlastBerryOverride に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(rows_ovr, hotfix_text, {"AthenaLootTierData_Client_BlastBerryOverride", "/BlastBerryLoot/DataTables/AthenaLootTierData_Client_BlastBerryOverride"})
    ...
    print(f"[FINAL MERGE] AthenaLT <- (BB, OVR) : replaced={rep_b+rep_o}, added={add_b+add_o}")
