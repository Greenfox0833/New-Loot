...
WEB_LOOTPOOL_NAME = "Reload_NoBuild_Ranked_ZB_LootPool.json"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_reload_nobuild_ranked")
    if logger.handlers:
        return logger
    ...
    logger.info("log file (Reload_NoBuild_Ranked): %s", log_path)
    return logger
...
def main():
    ...
    try:
        logger.info("===== Reload_NoBuild_Ranked: pipeline start =====")
        ...
        logger.info("build Reload_NoBuild_Ranked_LootData start")
        t0 = time.time()
        ...
        logger.info("✅ Reload_NoBuild_Ranked_LootData を作成: %s", br_out)
        ...
        try:
            ...
            msg = f"Reload_NoBuild_Ranked update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== Reload_NoBuild_Ranked: pipeline end =====")
