...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/BetterFishing_LootPool.json"

# Ensure child scripts use BetterFishing profile by default
os.environ.setdefault("SYSTEM_PROFILE", "BetterFishing")
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_BetterFishing")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== BetterFishing: pipeline start =====")
        ...
        try:
            ...
            msg = f"BetterFishing update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== BetterFishing: pipeline end =====")
