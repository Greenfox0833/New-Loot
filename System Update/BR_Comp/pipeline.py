...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_br_comp")
    if logger.handlers:
        return logger
    ...
    logger.info("log file (BR_Comp): %s", log_path)
    return logger
...
def main():
    ...
    try:
        logger.info("===== BR_Comp: pipeline start =====")
        ...
        try:
            web_out = get_web_lootpool_path(PROFILE_NAME)
            if web_out.exists():
        ...
        logger.info("===== BR_Comp: pipeline end =====")
