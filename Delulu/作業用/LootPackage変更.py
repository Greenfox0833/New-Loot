...
PARENT_PATHS = [
    Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/LootCurrentSeasonLootPackages_Client.json"),
    Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/NoBuildBR/OverrideLootPackagesData_NoBuildBR.json"),
    Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/LootCurrentSeason/Content/DataTables/Delulu/DeluluOverrideLootPackages_Client.json"),
]

HOTFIX_PATH  = Path("e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")

OUT_FINAL    = Path("E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Delulu/作業用/AthenaLootPackages_Client__final.json")
...
def main():
    ...
    # 2) ParentTables 順次マージ
    for p in PARENT_PATHS:
        meta = read_datatable_json(p)
        rows = meta["Rows"]
        rep, add = merge_rows(base_rows, rows)
        print(f"[MERGE] {p.name}: replaced={rep}, added={add}")
