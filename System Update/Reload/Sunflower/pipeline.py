...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/Sunflower_LootPool.json"

# Ensure child scripts resolve profile config at System Update/Reload/Sunflower
os.environ["SYSTEM_PROFILE"] = "Reload/Sunflower"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_Sunflower")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== Sunflower: pipeline start =====")
        ...
        try:
            ...
            msg = f"Sunflower update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== Sunflower: pipeline end =====")
