...
PATH_BB_NB  = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/NoBuild/AthenaLootPackages_Client_BlastBerryOverride_NoBuild.json")

# Hotfix
HOTFIX_PATH = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")

# 出力
OUT_FINAL   = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload_NoBuild/AthenaLootPackages_Client__final.json")
...
def main():
    ...
    meta_nb     = read_datatable_json(PATH_BB_NB)
    ...
    # ⑥ AthenaLootPackages_Client_BlastBerryOverride に AthenaLootPackages_Client_BlastBerryOverride_NoBuild を上書き
    rep, add = merge_rows(rows_ovr, rows_nb)
    print(f"[STEP6] BlastBerryOverride <- BlastBerryOverride_NoBuild : replaced={rep}, added={add}")

    # ⑦ AthenaLootPackages_Client_BlastBerryOverride_NoBuild に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(rows_nb, hotfix_text, {"AthenaLootPackages_Client_BlastBerryOverride_NoBuild", "/BlastBerryLoot/DataTables/NoBuild/AthenaLootPackages_Client_BlastBerryOverride_NoBuild"})
    ...
    print(f"[FINAL MERGE] Athena <- (BB, OVR, NB) : "
          f"replaced={rep_b+rep_o+rep_n}, added={add_b+add_o+add_n}")
