...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/ItemCache_03_LootPool.json"

# Ensure child scripts use ItemCache_03 profile by default
os.environ.setdefault("SYSTEM_PROFILE", "ItemCache_03")
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_ItemCache_03")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== ItemCache_03: pipeline start =====")
        ...
        try:
            ...
            msg = f"ItemCache_03 update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== ItemCache_03: pipeline end =====")
