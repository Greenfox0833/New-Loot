# BlastBerry_NoBuild_LT更新.py
# --- BlastBerry（NoBuild）用：LootTier 合体 + 個別Hotfix適用（完全版） ---
import json, re
...
# ==== 入出力（環境に合わせてパス調整OK）====
# ベース（Athena）
PATH_ATHENA_LT = Path(r"e:/Fmodel/Exports/FortniteGame/Content/Items/DataTables/AthenaLootTierData_Client.json")

# BlastBerry 一式（Tier）
PATH_BB_LT      = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/BlastBerryComposite_LT.json")
PATH_BB_LT_OVR  = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/AthenaLootTierData_Client_BlastBerryOverride.json")
PATH_BB_LT_NB   = Path(r"e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/BlastBerryLoot/Content/DataTables/NoBuild/AthenaLootTierData_Client_BlastBerryOverride_NoBuild.json")

# Hotfix（存在しなければ自動スキップ）
HOTFIX_PATH = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")

# 出力（最終合体ファイル）
OUT_FINAL   = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload_Nobuild/AthenaLootTierData_Client__final.json")
...
def parse_unreal_tuple_to_dict(s: str) -> Dict[str, Any]:
    ...
    for seg in inner.split(","):
        ...
        k, v = seg.split("=", 1)
        out[k].strip() if False else None  # no-op for style
...
def apply_hotfix_for_table(rows: Dict[str, Any], hotfix_text: str, table_keys) -> None:
    ...
    table_keys: 末尾名 / フルパスのどちらでも命中するように集合で渡す
    例:
      {"AthenaLootTierData_Client", "/Game/Items/Datatables/AthenaLootTierData_Client"}
    """
...
def main():
    # メタ読込
    meta_athena = read_datatable_json(PATH_ATHENA_LT)
    rows_athena = meta_athena["Rows"]

    meta_bb     = read_datatable_json(PATH_BB_LT)
    rows_bb     = meta_bb["Rows"]

    meta_ovr    = read_datatable_json(PATH_BB_LT_OVR)
    rows_ovr    = meta_ovr["Rows"]

    meta_nb     = read_datatable_json(PATH_BB_LT_NB)
    rows_nb     = meta_nb["Rows"]

    # Hotfixテキスト
    hotfix_text = HOTFIX_PATH.read_text(encoding="utf-8") if HOTFIX_PATH.exists() else ""

    # ① AthenaLootTierData_Client に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(
            rows_athena,
            hotfix_text,
            {"AthenaLootTierData_Client", "/Game/Items/Datatables/AthenaLootTierData_Client"}
        )
    ...
    # ② Athena に BlastBerryTierData を上書き
    rep, add = merge_rows(rows_athena, rows_bb)
    print(f"[STEP2] Athena <- BlastBerryTier : replaced={rep}, added={add}")

    # ③ BlastBerryTierData に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(
            rows_bb,
            hotfix_text,
            {"BlastBerryTierData", "/BlastBerryLoot/DataTables/BlastBerryTierData"}
        )

    # ④ BlastBerryTierData に AthenaLootTierData_Client_BlastBerryOverride を上書き
    rep, add = merge_rows(rows_bb, rows_ovr)
    print(f"[STEP4] BlastBerryTier <- BB_Override : replaced={rep}, added={add}")

    # ⑤ AthenaLootTierData_Client_BlastBerryOverride に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(
            rows_ovr,
            hotfix_text,
            {"AthenaLootTierData_Client_BlastBerryOverride",
             "/BlastBerryLoot/DataTables/AthenaLootTierData_Client_BlastBerryOverride"}
        )

    # ⑥ AthenaLootTierData_Client_BlastBerryOverride に AthenaLootTierData_Client_BlastBerryOverride_NoBuild を上書き
    rep, add = merge_rows(rows_ovr, rows_nb)
    print(f"[STEP6] BB_Override <- BB_Override_NoBuild : replaced={rep}, added={add}")

    # ⑦ AthenaLootTierData_Client_BlastBerryOverride_NoBuild に Hotfix
    if hotfix_text:
        apply_hotfix_for_table(
            rows_nb,
            hotfix_text,
            {"AthenaLootTierData_Client_BlastBerryOverride_NoBuild",
             "/BlastBerryLoot/DataTables/NoBuild/AthenaLootTierData_Client_BlastBerryOverride_NoBuild"}
        )

    # --- 最終合体：Athena に（Hotfix後の）BB → OVR → NB を順に反映 ---
    final_rows = rows_athena
    rep_b, add_b = merge_rows(final_rows, rows_bb)
    rep_o, add_o = merge_rows(final_rows, rows_ovr)
    rep_n, add_n = merge_rows(final_rows, rows_nb)
    print(f"[FINAL MERGE] Athena <- (BB, OVR, NB) : replaced={rep_b+rep_o+rep_n}, added={add_b+add_o+add_n}")
