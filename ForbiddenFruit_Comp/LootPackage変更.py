...
OVERRIDE_NOBUILD_PATH = Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/ForbiddenFruitDataTables/Content/DataTables/AthenaLootPackages_Client_ForbiddenFruitChapterOverride_NoBuild.json")  # ②上書き
OVERRIDE_SHOWDOWN_PATH = Path("e:/Fmodel/Exports/FortniteGame/Plugins/GameFeatures/ForbiddenFruitDataTables/Content/DataTables/Showdown/NoBuild/AthenaLootPackages_Client_ForbiddenFruitChapterOverride_NoBuild_Showdown.json")  # ③最終上書き
HOTFIX_PATH = Path("e:/フォートナイト/Picture/Loot Pool/TEST4/Hotfix/Hotfix.ini")  # ①でのみ適用
OUT_FINAL   = Path("E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit_Comp/AthenaLootPackages_Client__final.json")

# ①のHotfix対象テーブル名（Hotfixはベースにのみ適用する意図を厳密化）
...
def merge_rows(base_rows: Dict[str, Any], override_rows: Dict[str, Any]) -> Tuple[int, int]:
    """override_rowsをbase_rowsへ上書きマージ"""
    replaced = added = 0
...
def parse_hotfix_line(line: str) -> Dict[str, Any]:
    # +DataTable=...;RowUpdate;RowKey;Field;Value  / RowAdd / RowUpsert / RowDelete
    line = line.strip()
...
def apply_hotfix(rows: Dict[str, Any], hotfix_text: str, allowed_table_names: Tuple[str, ...]) -> None:
    """allowed_table_namesに一致するDataTableに対する行だけをrowsへ適用"""
    print("[HOTFIX] start")
    ...
    for ln, line in enumerate(hotfix_text.splitlines(), 1):
        ...
        dt = h.get("datatable", "")
        dt_lower = dt.lower()
        if not any(name.lower() in dt_lower or dt_lower.endswith(name.lower()) for name in allowed_table_names):
            # 対象外テーブルは完全スキップ
            continue
...
# ---------- メイン ----------
def main():
    # 1) ベース（ForbiddenFruitChapterLootPackages）読込
    ...
    nobuild_rows = nobuild_meta["Rows"]
    rep, add = merge_rows(base_rows, nobuild_rows)
    print(f"[MERGE Override_NoBuild] replaced={rep}, added={add}")

    # 4) Showdown 上書き（最終）
    showdown_meta = read_datatable_json(OVERRIDE_SHOWDOWN_PATH)
    showdown_rows = showdown_meta["Rows"]
    rep, add = merge_rows(base_rows, showdown_rows)
