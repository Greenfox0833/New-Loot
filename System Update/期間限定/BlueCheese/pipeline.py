...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/BlueCheese_LootPool.json"

# Ensure child scripts use BlueCheese profile
os.environ["SYSTEM_PROFILE"] = "BlueCheese"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_BlueCheese")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== BlueCheese: pipeline start =====")
        ...
        try:
            ...
            msg = f"BlueCheese update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== BlueCheese: pipeline end =====")
