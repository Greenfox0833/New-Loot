...
INPUT_MINLIST_JSON = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/PiperBoot/items_unique_min.json"


# 画像の保存先（親）:  <OUTPUT_BASE_DIR>/<TierGroup>/<WorldListKey>/ に振り分け保存
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/アイテム画像/Reload/PiperBoot"
...
def main():
    ...
    loot_summary_py  = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/PiperBoot/LootSummary.py")
    version_save_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/PiperBoot")  # まとめJSONの保存先
    lt_json_path     = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/PiperBoot/AthenaLootTierData_Client__final_LT.json")
    lp_json_path     = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/PiperBoot/AthenaLootPackages_Client__final_LP.json")
    ...
    try:
        ...
        # 0) Hotfix（必要時のみ実行）
        if DO_HOTFIX:
            subprocess.run([sys.executable, r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Reload/PiperBoot/LootPackage更新.py"], check=True)
        ...
        br_lootdata_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/Reload/PiperBoot/LootPercent")
        br_lootdata_dir.mkdir(parents=True, exist_ok=True)
        br_out = br_lootdata_dir / f"Reload_PiperBoot_LootData_{br_now}.json"
        ...
        print(f"✅ Reload_PiperBoot_LootData を作成: {br_out}")
        ...
        try:
            ...
            msg = f"Reload PiperBoot update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        print("===== Reload PiperBoot: pipeline end =====")
