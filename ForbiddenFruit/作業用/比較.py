...
old_file = Path(sys.argv[1]) if len(sys.argv) >= 3 else Path("e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/Summary/v37.00_v5.json")
new_file = Path(sys.argv[2]) if len(sys.argv) >= 3 else Path("e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/Summary/v37.00_v6.json")
        ...
        if isinstance(x, dict):
            if "ListItems" in x and isinstance(x["ListItems"], list):
                for li in x["ListItems"]:
                    ...
                    # LocalizedName を最優先に追加し、他の候補も順番にチェック
                    name = (li.get("LocalizedName") or li.get("Localized") or
                            li.get("Name_JA") or li.get("Name") or
                            li.get("RowName") or "???")
                    prefer_put(ap, name)
