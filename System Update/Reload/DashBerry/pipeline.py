...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/DashBerry_LootPool.json"

# Ensure child scripts resolve profile config at System Update/Reload/DashBerry
os.environ["SYSTEM_PROFILE"] = "Reload/DashBerry"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_DashBerry")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== DashBerry: pipeline start =====")
        ...
        try:
            ...
            msg = f"DashBerry update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== DashBerry: pipeline end =====")
