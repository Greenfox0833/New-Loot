...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/ZB_Comp_LootPool.json"

# Ensure child scripts use NoBuild_Comp profile by default
os.environ.setdefault("SYSTEM_PROFILE", "NoBuild_Comp")
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_nobuild_comp")
    if logger.handlers:
        return logger
    ...
    logger.info("log file (NoBuild_Comp): %s", log_path)
    return logger
...
def main():
    ...
    try:
        logger.info("===== NoBuild_Comp: pipeline start =====")
        ...
        logger.info("build NoBuild_LootData start")
        t0 = time.time()
        ...
        logger.info("✅ NoBuild_LootData を作成: %s", br_out)
        if WEB_LOOTPOOL_OUT:
            try:
        ...
        try:
            ...
            msg = f"NoBuild_Comp update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== NoBuild_Comp: pipeline end =====")
