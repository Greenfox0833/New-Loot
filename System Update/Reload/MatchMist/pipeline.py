...
WEB_LOOTPOOL_NAME = "MatchMist_LootPool.json"

# Ensure child scripts resolve profile config at System Update/Reload/MatchMist
os.environ["SYSTEM_PROFILE"] = "Reload/MatchMist"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_MatchMist")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== MatchMist: pipeline start =====")
        ...
        try:
            ...
            msg = f"MatchMist update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== MatchMist: pipeline end =====")
