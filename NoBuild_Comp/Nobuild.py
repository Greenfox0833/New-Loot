...
INPUT_LP_JSON = r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/NoBuild_Comp/AthenaCompositeLP_NoBuildBR_Comp_Merged_Hotfix.json"

# 画像の保存先（親）:  <OUTPUT_BASE_DIR>/<TierGroup>/<WorldListKey>/ に振り分け保存
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/NoBuild_Comp/v37.00"
...
import subprocess
if DO_HOTFIX:
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/NoBuild_Comp/LP_NoBuildLP合体.py"], check=True)
# --- Hotfix適用ここまで ---
...
def build_summary(rows_lt: dict, rows_lp: dict):
    # 各行が dict でない場合をスキップ
    id_to_call = {k: (v.get("LootPackageCall", "") if isinstance(v, dict) else "")
                  for k, v in rows_lp.items()}
    ...
    for row_key, row in rows_lp.items():
        ...
        cr = row.get("CountRange")
        count_x = cr.get("X") if isinstance(cr, dict) else None
...
def main():
    ...
    versioned_filename = get_next_version_filename(
        VERSION_PREFIX,
        r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/NoBuild_Comp/Summary"
    )
