...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/PunchBerry_LootPool.json"

# Ensure child scripts resolve profile config at System Update/Reload/PunchBerry
os.environ["SYSTEM_PROFILE"] = "Reload/PunchBerry"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_PunchBerry")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== PunchBerry: pipeline start =====")
        ...
        try:
            ...
            msg = f"PunchBerry update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== PunchBerry: pipeline end =====")
