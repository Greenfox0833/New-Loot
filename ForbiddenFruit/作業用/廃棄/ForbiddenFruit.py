...
INPUT_LT_JSON = r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/Updated_LootTier.json"
INPUT_LP_JSON = r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/Updated_LootPackages.json"

# まとめ出力（summary.json）
OUTPUT_JSON = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/summary.json"

# 画像の保存先（親）:  <OUTPUT_BASE_DIR>/<TierGroup>/<WorldListKey>/ に振り分け保存
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/Week9 - Phase1"
...
import subprocess
if DO_HOTFIX:
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/LootPackage更新.py"], check=True)
    subprocess.run(["python", r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/ForbiddenFruit/作業用/LootTier更新.py"], check=True)
# --- Hotfix適用ここまで ---
...
DEBUG_LOCALIZE = True  # True にすると取得ログを出す
...
@atexit.register
def _save_localize_cache():
    """終了時にキャッシュを必ず保存"""
    with _serialize_lock:
        try:
            with open(LOCALIZE_CACHE_FILE, "w", encoding="utf-8") as f:
                json.dump(LOCALIZE_CACHE, f, ensure_ascii=False, indent=2)
        except Exception:
            pass
    ...
    try:
        ...
        def pick_icon(entry):
            if isinstance(entry, dict):
                ...
                if "WideIcon" in entry and "AssetPathName" in entry["WideIcon"]:
                    return entry["WideIcon"]["AssetPathName"]
            return None
...
def main():
    ...
    print(f"✅ JSONファイルを作成しました: {OUTPUT_JSON}")

    # 3) タスク収集（TierGroup/WorldListごと保存）
