...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/assets/data/Loot/StintBrush_LootPool.json"

# Ensure child scripts use StintBrush profile
os.environ["SYSTEM_PROFILE"] = "StintBrush"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_StintBrush")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== StintBrush: pipeline start =====")
        ...
        t0 = time.time()
        item_metadata_filename = PROJECT_ROOT / "shared" / "cache" / "item_metadata.json"
        ...
        try:
            ...
            msg = f"StintBrush update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== StintBrush: pipeline end =====")
