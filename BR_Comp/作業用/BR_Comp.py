...
SPECIAL_LIST_PERCENT_RULES = {
    "Loot_AthenaFloorLoot": {
        ...
        "WorldPKG.AthenaLoot.Weapon.SMG.01",
...
INPUT_MINLIST_JSON = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/items_unique_min.json"
...
def main():
    ...
    loot_summary_py  = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/LootSummary.py")
    version_save_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/BR_Comp")  # まとめJSONの保存先
    lt_json_path     = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/AthenaLootTierData_Client__final.json")
    lp_json_path     = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/AthenaLootPackages_Client__final.json")
    ...
    try:
        ...
        # 0) Hotfix（必要時のみ実行）
        if DO_HOTFIX:
            subprocess.run([sys.executable, r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/LootPackage変更.py"], check=True)
            subprocess.run([sys.executable, r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR_Comp/作業用/LootTier変更.py"], check=True)
            print("✓ Hotfix 適用完了")
        ...
        try:
            ...
            msg = f"BR_Comp update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
