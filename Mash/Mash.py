...
INPUT_MINLIST_JSON = r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Mash/items_unique_min.json"


# 画像の保存先（親）:  <OUTPUT_BASE_DIR>/<TierGroup>/<WorldListKey>/ に振り分け保存
OUTPUT_BASE_DIR = r"E:/フォートナイト/Picture/Loot Pool/TEST4/アイテム画像/Mash"
...
def main():
    ...
    loot_summary_py  = Path(r"e:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Mash/LootSummary.py")
    version_save_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/Mash")  # まとめJSONの保存先
    lt_json_path     = Path(r"e:/Fmodel/Exports/FortniteGame/Content/Items/DataTables/AthenaLootTierData_Client.json")
    lp_json_path     = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Mash/AthenaLootPackages_Client__final.json")
    minlist_path     = Path(INPUT_MINLIST_JSON)  # 例: E:/.../BR/作業用/items_unique_min.json

    try:
        print("===== Mash: pipeline start =====")

        # 0) Hotfix（必要時のみ実行）
        if DO_HOTFIX:
            subprocess.run([sys.executable, r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/Mash/LootPackage変更.py"], check=True) #
            subprocess.run([sys.executable, r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/BR/作業用/LootTier変更.py"], check=False) #
            print("✓ Hotfix 適用完了")
        ...
        br_lootdata_dir = Path(r"E:/フォートナイト/Picture/Loot Pool/TEST4/New Loot/戦利品データ/Mash/LootPercent")
        br_lootdata_dir.mkdir(parents=True, exist_ok=True)
        br_out = br_lootdata_dir / f"Mash_LootData_{br_now}.json"
        ...
        print(f"✅ Mash_LootData を作成: {br_out}")
        ...
        try:
            ...
            msg = f"Mash update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
