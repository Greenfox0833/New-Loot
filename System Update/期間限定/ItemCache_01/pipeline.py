...
# Ensure child scripts use ItemCache_01 profile by default
os.environ.setdefault("SYSTEM_PROFILE", "ItemCache_01")
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_ItemCache_01")
    if logger.handlers:
        return logger
...
def main():
    ...
    try:
        logger.info("===== ItemCache_01: pipeline start =====")
        ...
        try:
            ...
            msg = f"ItemCache_01 update {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        logger.info("===== ItemCache_01: pipeline end =====")
