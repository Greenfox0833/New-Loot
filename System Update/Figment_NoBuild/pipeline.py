...
WEB_LOOTPOOL_OUT = r"E:/フォートナイト/Web/loot/data/ORIGIN_ZB_LootPool.json"
...
def _setup_logger():
    ...
    logger = logging.getLogger("pipeline_figment_nobuild")
    if logger.handlers:
        return logger
    ...
    logger.info("log file (Figment_NoBuild): %s", log_path)
    return logger
...
def main():
    ...
    try:
        logger.info("===== Figment_NoBuild: pipeline start =====")
        ...
        logger.info("build Figment_NoBuild_LootData start")
        t0 = time.time()
        ...
        logger.info("===== Figment_NoBuild: pipeline end =====")
